...
with col2:
                ...
                try:
                    ...
                    if update_result:
                        if enabled:
                            ...
                            try:
                                TaskScheduled.delete_task(task_name)
                            except Exception: # Catch specific errors if known, else generic
                                pass # Task might not exist, which is fine
                            st.success("Settings saved! Scheduling disabled.")

                        # Removed automatic message sending on save
                        t.sleep(2)
                        st.rerun()
                    else:
    ...
    # else: # Handles cases where ut might be None (initialization failed)
        # Error already shown at the top
                    st.error(f"Error configuring schedule: {str(e)}")
                    st.exception(e) # Show full traceback for debugging
    elif not group_map and ut is not None: # if group_map is empty but ut is initialized
         st.warning("No groups found or system not initialized correctly!")
